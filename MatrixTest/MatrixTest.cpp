...
TEST(Matrix, can_creation_of_a_matrix_of_default_size)
{
  ASSERT_NO_THROW(TMatrix <int> M());
}


TEST(Matrix, can_creation_of_a_matrix_of_specified_size)
{
...
{
  ASSERT_ANY_THROW(TMatrix <int> M(10000));
}
...
TEST(Matrix, can_equality_matrix_with_different_size)
{
...
TEST(Matrix, can_inequality_matrix_with_different_size)
{
  TMatrix <int> M1(2), M2(3);
  ASSERT_TRUE(M1 != M2);
}
...
TEST(Matrix, thorws_when_addition_matrix_with_different_size)
{
  TMatrix <int> M1(2), M2(3);
...
TEST(Matrix, thorws_when_subtraction_matrix_with_different_size)
{
  TMatrix <int> M1(2), M2(3);
  ASSERT_ANY_THROW(M1 - M2);
}
...
TEST(Matrix, thorws_when_delit_matrix_with_different_size)
{
  TMatrix <int> M1(2), M2(3);
  ASSERT_ANY_THROW(M1 / M2);
}

TEST(Matrix, thorws_when_multiplication_matrix_with_different_size)
{
  TMatrix <int> M1(2), M2(3);
  ASSERT_ANY_THROW(M1 * M2);
}
TEST(Matrix, can_do_multiple_operations_at_once)
{
	TMatrix <int> M1(3), M2(3), M3(3), Temp(3);
	int k = 1;
	for (int i = 0; i < 3; i++)
		for (int j = 0; j < 3 - i; j++)
		{
